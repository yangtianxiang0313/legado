...
class LocalApprovalSession:
    def __init__(
        ...
        *,
        ...
        def approve_from_http(supplied_token: str) -> str:
            validate_token(supplied_token)
            with self.harness.mutation_lock():
                ...
                if current.binding() != self.snapshot.binding():
                    raise ApprovalConflict("验收页打开后候选或 Evidence 已变化")

                now = self.clock()
                approved_at = iso8601_utc(now)
                ...
                transaction = ApprovalFileTransaction(records)
                transaction.commit()
                try:
                    outcome = self.harness.close(current.item_id)
                    if outcome != "completed":
                        raise ApprovalRecoveryRequired(
                            f"Approval 已记录，但 close 尚未完成：{outcome}"
                        )
        ...
        self.server = LoopbackHTTPServer(
            (LOOPBACK_HOST, 0),
            self._handler_type(
                approve_callback=approve_from_http,
    ...
    def _handler_type(
        self,
        *,
        approve_callback: Callable[[str], str],
            ...
            def do_POST(self) -> None:
                ...
                try:
                    if self.path == "/api/approve":
                        outcome = approve_callback(supplied_token)
                    else:
                    ...
                    session.outcome = "failed"
                    self._send_json(500, {"error": "审批处理失败，任务未完成"})
                    return
    ...
    def render_page(self) -> str:
        ...
        gate_rows = "".join(
            f"<li><code>{html.escape(gate)}</code></li>" for gate in snapshot.gates
        )
        file_rows = "".join(
            "<tr>"
...
<head>
  ...
  <style nonce="{html.escape(self.page_nonce)}">
    ...
    #approve {{ background: #1677ff; color: white; }}
...
<body>
<main>
  <h1>人工验收：{html.escape(snapshot.title)}</h1>
  <p><code>{html.escape(snapshot.item_id)}</code> · attempt {snapshot.attempt}</p>
  <section class="warn">
    <strong>本地开发审批（local_unverified）</strong>
    <p>按钮会代你生成 Approval 文件，但本机账号不是强身份，signature 为 null。发布级 Gate 仍须由受保护 CI/SSO 或 Touch ID/FIDO2 helper 重验。</p>
  </section>
  <section>
    <h2>需要批准的 Gate</h2>
    <ul>{gate_rows}</ul>
  ...
  <section>
    <h2>冻结文件（{len(snapshot.review_subject_files)}）</h2>
    <table><thead><tr><th>路径</th><th>内容摘要</th></tr></thead><tbody>{file_rows}</tbody></table>
  </section>
  <section>
    <button id="approve" type="button">批准全部 Gate 并完成任务</button>
    <button id="cancel" type="button">暂不批准</button>
...
(() => {{
  ...
  if (!token) {{
    ...
    result.textContent = "审批会话 token 缺失，请重新运行 Harness review。";
    return;
  }}
  cancel.addEventListener("click", async () => {{
    approve.disabled = true;
    cancel.disabled = true;
    result.textContent = "正在结束本次审批会话…";
    try {{
      ...
      result.textContent = "未批准，会话已结束；未写入任何 Approval。";
    }} catch (error) {{
  ...
  approve.addEventListener("click", async () => {{
    ...
    try {{
      ...
      if (payload.outcome === "completed") {{
        result.textContent = "已批准并完成任务，可以关闭此页面。";
      }} else if (payload.outcome === "recovery_required") {{
        result.textContent = "人工决定已记录，但 Harness 关闭事务需要恢复；请保留页面并查看终端。";
      }} else {{
        throw new Error(`未知结果：${{payload.outcome}}`);
      }}
    }} catch (error) {{
      result.textContent = `未批准：${{error.message}}`;
    }}
  }});
}})();
</script>
        ...
        *,
        ...
        try:
            ...
            if self.outcome != "completed":
                raise ApprovalUIError(f"本地审批未完成：{self.outcome}")
            return self.outcome
        finally:
            self.server.server_close()
...
def run_local_review(harness: Any, item_id: str) -> str:
    ...
    print("本地审批页已在浏览器打开；等待人工点击（授权 token 不会输出）…")
