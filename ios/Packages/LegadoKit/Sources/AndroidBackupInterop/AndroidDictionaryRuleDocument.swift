...
public struct AndroidDictionaryRuleDTO: AndroidBackupJSONDocument {
  public static let knownFieldNames: Set<String> = [
    "name", "urlRule", "showRule", "enabled", "sortNumber",
  ]
  ...
  public init(values: [String: JSONValue?], unknownFields: [String: JSONValue] = [:]) {
...
public enum AndroidDictionaryRuleCodec {
    ...
    -> [AndroidDictionaryRuleDTO]
  {
    try AndroidBackupDocumentCodec.decodeMany(
      AndroidDictionaryRuleDTO.self,
      from: data,
      maximumDepth: maximumDepth
    )
  }

  public static func encodeMany(_ values: [AndroidDictionaryRuleDTO]) throws -> Data {
