  ...
  public static func run(
    ...
    let fixtureRoot =
      fixtureID == ReaderBookmarkConformanceRunner.fixtureID
        ...
        || fixtureID
          == ReaderOfflineCacheQueueConformanceRunner.fixtureID
        || fixtureID
          == ReaderContentAcquisitionConformanceRunner.fixtureID
        || fixtureID
          == ReaderContentNormalizationConformanceRunner.fixtureID
        ...
        || fixtureID == ReaderTOCHandoffConformanceRunner.fixtureID
      ? "runtime-lab"
      : "source-lab"
    ...
    if fixtureID == ReaderReadRecordConformanceRunner.fixtureID {
      let run = try ReaderReadRecordConformanceRunner.run(
        fixtureDirectory: fixtureDirectory
      )
      return try finish(
        taskID: taskID,
    ...
    if fixtureID == ReaderProgressSaveConformanceRunner.fixtureID {
      let run = try ReaderProgressSaveConformanceRunner.run(
        fixtureDirectory: fixtureDirectory
      )
      return try finish(
        taskID: taskID,
    ...
    if fixtureID == ReaderOfflineCacheQueueConformanceRunner.fixtureID {
      let run = try ReaderOfflineCacheQueueConformanceRunner.run(
        fixtureDirectory: fixtureDirectory
      )
      return try finish(
        taskID: taskID,
    ...
    if fixtureID
      == ReaderContentNormalizationConformanceRunner.fixtureID
    {
      let run = try ReaderContentNormalizationConformanceRunner.run(
        fixtureDirectory: fixtureDirectory
      )
      return try finish(
        taskID: taskID,
    ...
    if fixtureID == ReaderSessionResetConformanceRunner.fixtureID {
      let run = try ReaderSessionResetConformanceRunner.run(
        fixtureDirectory: fixtureDirectory
      )
      return try finish(
        taskID: taskID,
    ...
    if fixtureID == BookDetailActionConformanceRunner.fixtureID {
    ...
    if fixtureID == SearchBookLifecycleConformanceRunner.fixtureID {
      let run = try SearchBookLifecycleConformanceRunner.run(
        fixtureDirectory: fixtureDirectory
      )
      return try finish(
        taskID: taskID,
    ...
    if fixtureID == SourceEditorDebugConformanceRunner.fixtureID {
      let run = try SourceEditorDebugConformanceRunner.run(
        fixtureDirectory: fixtureDirectory
      )
      return try finish(
        taskID: taskID,
    ...
    if fixtureID == SourceImportConformanceRunner.fixtureID {
      let run = try SourceImportConformanceRunner.run(
        fixtureDirectory: fixtureDirectory
      )
      return try finish(
        taskID: taskID,
    ...
    if fixtureID == ReaderTOCHandoffConformanceRunner.fixtureID {
      let run = try ReaderTOCHandoffConformanceRunner.run(
        fixtureDirectory: fixtureDirectory
      )
      return try finish(
        taskID: taskID,
    ...
    if fixtureID == SourceJSONPathRegexConformanceRunner.fixtureID {
      let run = try SourceJSONPathRegexConformanceRunner.run(
        fixtureDirectory: fixtureDirectory
      )
      return try finish(
        taskID: taskID,
  ...
  private static func comparisonValue(
    ...
    guard
      ...
      result["type"] == .string("source_pipeline")
        ...
        || result["type"] == .string("library_runtime"),
