...
3. 一个 PR 最多变更一个核心依赖。
4. 新依赖/升级必须使用独立 Task 和 ADR，记录用途、替代方案、传递依赖、许可证、Privacy Manifest、维护状态、包体增量、退出方案和责任人。
...
普通产品 Task 不能修改 Package manifest、lock 或 policy。依赖 Task 只处理一个候选，完成
解析、传递依赖、许可证、Privacy Manifest、安全、包体和退出方案检查后，才能更新
policy、`Package.swift`、`Package.resolved`、SBOM 和 notices。随后普通验收恢复禁止自动解析。
